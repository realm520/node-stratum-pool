        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            options.poolAddressScript = (function(){
                switch(options.coin.reward){
                    ...
                    case 'POW':
                        return util.P2SH(rpcResults.validateaddress.address);
                }
            })();
    ...
    function StartStratumServer(finishedCallback){
        ...
        _this.stratumServer.on('started', function(){
            ...
            client.on('difficultyChanged', function(diff){
                ...
                console.log("submit share: ", params);
    ...
    function GetBlockTemplate(callback){
        _this.daemon.cmd('getblocktemplate',
            ...
            function(result){
                ...
                } else if (options.coin.name == 'UnitedBitcoin') {
                    let blockTpl = result.response;
                    _this.daemon.cmd('getcoinbase',
                        [options.poolAddressScript.toString('hex'), result.response.coinbase.data],
                        function(result) {
                            // console.log(result);
                            if (result.error) {
                                emitErrorLog('getblocktemplate call failed for daemon instance ' +
                                ...
                                blockTpl.coinbase.data = result.response.data;
                                blockTpl.coinbase.txid = result.response.txid;
                                blockTpl.coinbase.hash = result.response.hash;
                                var processedNewBlock = _this.jobManager.processTemplate(blockTpl);
                                callback(null, blockTpl, processedNewBlock);
                                callback = function(){};
                            }
                        }, true
                    );
